...
/* Mutliple output regression trees
 */
...
/* particularise à ce problème certaines fonctions de tree-model-weight-symb-missing.c */
...
DllExport int allocate_multiregr_table_score(int nb_goal) {
...
/* float getobjy_multiregr(int obj, int i) { */
/*   return getattval(obj,goal_multiregr[i]); */
/* } */
...
void init_multiregr_problem(int *att_vect, int nb_att, int *goal_mr, int nb_goal_mr) {
  goal_type=MULTIREGR;
  attribute_vector=att_vect;
  nb_attributes=nb_att;
  goal_multiregr=goal_mr;
  nb_goal_multiregr=nb_goal_mr;
  nb_classes=0;
#ifdef LOAD_DB_HANDLER
  if (core_table_type==SPARSE_CORE_TABLE) {    
    getattval=getattval_sparse;
    getobjy_multiregr_learn=getobjy_multiregr_learn_true;
...
void set_find_a_threshold_num_function_multiregr(int i, int nb_rt) {
...
void set_find_a_threshold_symb_function_multiregr(int i) {
  if (i==1) {
    find_a_threshold_symb=find_a_threshold_symb_at_random_multiregr;
  } else {
...
/* get_tree_object_weight(int tree, int obj) -> tree-distance.c */
/* get_ens_object_weight(int obj) -> tree-distance.c */
/* get_tree_node_object_weight(int node) -> tree-distance.c */
