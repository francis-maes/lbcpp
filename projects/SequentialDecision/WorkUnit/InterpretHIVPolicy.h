...
{
public:
  InterpretHIVPolicy() : numInitialStates(100), maxHorizon(300), discount(0.98) {}
  ...
  virtual Variable run(ExecutionContext& context)
  {
    ...
    size_t searchBudget;
    FunctionPtr searchHeuristic = createSearchHeuristic(context, problem, searchBudget);
    ScalarVariableStatisticsPtr returnStatistics = new ScalarVariableStatistics("return");

    std::pair<ContainerPtr, ContainerPtr> examples = createLearningExamples(context, problem, searchHeuristic, searchBudget, initialStates, returnStatistics);
    ...
    testDiscoveredPolicy(context, initialStates, a1, a2);
    return true;
  }
...
protected:
  ...
  FunctionPtr createSearchHeuristic(ExecutionContext& context, DecisionProblemPtr problem, size_t& budget)
  {
...
0.894523602 1.19929614 0.993329871 -0.0677094665 0.696950242 3.15939044 -0.630735484 -0.0164870716 -0.399100524 0.609717444 -1.05809382 0.0425629618 0.494840122 -0.110886859 -1.23804899 -0.80566502 -0.624816906 -1.05789091
    */
    ...
    return new HIVSearchHeuristic(featuresFunction, parameters);
  }

  std::pair<ContainerPtr, ContainerPtr> createLearningExamples(ExecutionContext& context, const DecisionProblemPtr& problem, const FunctionPtr& searchHeuristic, size_t searchBudget, const ContainerPtr& initialStates, ScalarVariableStatisticsPtr returnStatistics)
  {
    File outputFile = context.getFile("samples" + String((int)maxHorizon) + T("_") + String((int)numInitialStates) + T(".txt"));
    OutputStream* ostr = outputFile.createOutputStream();

    SearchPolicyPtr searchPolicy = new BestFirstSearchPolicy(searchHeuristic);
    ...
    for (size_t i = 0; i < initialStates->getNumElements(); ++i)
    {
      ...
      for (size_t t = 0; t < maxHorizon; ++t)
      {
        ...
        SearchTreePtr searchTree = new SearchTree(problem, state, searchBudget);
        searchTree->doSearchEpisode(context, searchPolicy, searchBudget);
        ...
        double costs[4];
        costs[0] = !a1 && !a2 ? 0.0 : 1.0;
        costs[1] = a1 && !a2 ? 0.0 : 1.0;
        costs[2] = !a1 && a2 ? 0.0 : 1.0;
        costs[3] = a1 && a2 ? 0.0 : 1.0;
        for (size_t j = 0; j < 4; ++j)
          line += String(costs[j]) + T(" ");
        line += T("\n");
        *ostr << line;
        ostr->flush();
  ...
  void testDiscoveredPolicy(ExecutionContext& context, const ContainerPtr& initialStates, const LuapeBinaryClassifierPtr& a1, const LuapeBinaryClassifierPtr& a2)
  {
    context.enterScope(T("Testing policy ") + a1->getRootNode()->toShortString() + T("; ") + a2->getRootNode()->toShortString());
