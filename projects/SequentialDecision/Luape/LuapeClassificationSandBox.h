...
public:
  virtual Variable run(ExecutionContext& context)
  {
    distributedContext = distributedExecutionContext(context, "monster24.montefiore.ulg.ac.be", 1664, "FormulaBoosting", "Francis", "fmaes@nic3", fixedResourceEstimator(1, 2048, 24));
  ...
  virtual void workUnitFinished(const WorkUnitPtr& workUnit, const Variable& result)
  {
    ...
    String method = workUnit->getVariable(cl->findMemberVariable("method")).getString();
    String variant = workUnit->getVariable(cl->findMemberVariable("variant")).getString();
    if (result.isBoolean())
    {
