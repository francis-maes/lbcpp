...
function testFunction(x)
  x = (x + 1) / 2
  print (x)
  local index = 1 + math.floor(x * 100)
  histogram[index] = histogram[index] + 1

  local p = (math.sin(13 * x) * math.sin(27 * x) + 1) / 2
  return Stochastic.bernoulli(p)
end

local objective = lbcpp.LuaFunction.create(testFunction, "Double", "Double")
local optimizer = Optimizer.HOO{numIterations=10000,nu=1,rho=0.5}
local score,solution = optimizer{objective = objective}
