...
#ifndef LCPP_CORE_IMPL_POLICY_STATIC_H_
# define LCPP_CORE_IMPL_POLICY_STATIC_H_
...
struct Policy : public Object<ExactType>
{
  ...
  std::string toString() const
  {
    ...
    std::string res = lcpp::toString(n) + " result(s):\n";
    for (size_t i = 0; i < n; ++i)
    {
      ObjectPtr result = BaseClass::_this().getResult(i);
      assert(result);
      res += "\t" + lcpp::toString(i) + ")\t" + result->getName() + "\t" + lcpp::toString(BaseClass::_this().getResult(i)) + "\n";
    }
    return res;
  }
...
#endif // !LCPP_CORE_IMPL_POLICY_STATIC_H_
