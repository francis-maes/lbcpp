...
#ifndef LCPP_CORE_IMPL_FUNCTION_MULTICLASS_LOG_BINOMIAL_LOSS_FUNCTION_H_
# define LCPP_CORE_IMPL_FUNCTION_MULTICLASS_LOG_BINOMIAL_LOSS_FUNCTION_H_
...
struct MultiClassLogBinomialLossFunction : public ScalarVectorFunction< MultiClassLogBinomialLossFunction >
{
  ...
  void compute(const FeatureGeneratorPtr input, double* output, const FeatureGeneratorPtr, FeatureGeneratorPtr* gradient) const
  {
    ...
    {
      std::cerr << "LogZ is not a valid number. Scores: " << lcpp::toString(scores) << std::endl;
      assert(false);
    }
    if (output)
...
#endif // !LCPP_CORE_IMPL_FUNCTION_MULTICLASS_LOG_BINOMIAL_LOSS_FUNCTION_H_
