...
static CTypeID cp_decl_constinit(CPState *cp, CType **ctp, CTypeID typeid_)
{
  CType *ctt = ctype_get(cp->cts, typeid_);
  CTInfo info;
  CTSize size;
  CPValue k;
  CTypeID constid;
  while (ctype_isattrib(ctt->info)) {  /* Skip attributes. */
    typeid_ = ctype_cid(ctt->info);  /* Update ID, too. */
    ctt = ctype_get(cp->cts, typeid_);
  }
  info = ctt->info;
  size = ctt->size;
  ...
  (*ctp)->info = CTINFO(CT_CONSTVAL, CTF_CONST|typeid_);
  k.u32 <<= 8*(4-size);
...
static CTypeID cp_decl_struct(CPState *cp, CPDecl *sdecl, CTInfo sinfo)
{
  ...
  if (cp_opt(cp, '{')) {  /* Struct/union definition. */
	...
	typeid_ = cp_decl_intern(cp, &decl);

	if ((scl & CDF_STATIC)) {  /* Static constant in struct namespace. */
	  CType *ct;
	  CTypeID fieldid = cp_decl_constinit(cp, &ct, typeid_);
	  ...
	  CType *tct = ctype_raw(cp->cts, typeid_);
	  ...
	  if (decl.bits == CTSIZE_INVALID) {  /* Regular field. */
		...
		cp_err_token(cp, CTOK_IDENT);
	      ct->info = CTINFO(CT_ATTRIB, CTATTRIB(CTA_SUBTYPE) + typeid_);
	  ...
	  ct->info = CTINFO(CT_FIELD, typeid_ + (bsz << CTSHIFT_BITCSZ));
	  ct->size = decl.attr;
...
static void cp_decl_func(CPState *cp, CPDecl *fdecl)
{
  ...
  if (cp->tok != ')') {
	...
	anchor = fieldid;
      lastid = fieldid;
      if (decl.name) ctype_setname(ct, decl.name);
      ct->info = CTINFO(CT_FIELD, typeid_);
      ct->size = nargs++;
    } while (cp_opt(cp, ','));
  }
  cp_check(cp, ')');
...
static void cp_decl_multi(CPState *cp)
{
  ...
  while (cp->tok != CTOK_EOF) {
	...
	if ((scl & CDF_TYPEDEF)) {  /* Create new typedef. */
	  ...
	  ct->info = CTINFO(CT_TYPEDEF, typeid_);
	  goto noredir;
	} else if (ctype_isfunc(ctype_get(cp->cts, typeid_)->info)) {
	  /* Treat both static and extern function declarations as extern. */
	  ct = ctype_get(cp->cts, typeid_);
	  /* We always get new anonymous functions (typedefs are copied). */
	  lua_assert(gcref(ct->name) == NULL);
	  id = typeid_;  /* Just name it. */
	} else if ((scl & CDF_STATIC)) {  /* Accept static constants. */
	  id = cp_decl_constinit(cp, &ct, typeid_);
	  goto noredir;
	} else {  /* External references have extern or no storage class. */
	  id = lj_ctype_new(cp->cts, &ct);
	  ct->info = CTINFO(CT_EXTERN, typeid_);
	}
