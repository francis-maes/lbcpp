...
#ifndef LBCPP_CORE_IMPL_POLICY_STATIC_H_
# define LBCPP_CORE_IMPL_POLICY_STATIC_H_
...
struct Policy : public Object<ExactType>
{
  ...
  std::string toString() const
  {
    ...
    std::string res = lbcpp::toString(n) + " result(s):\n";
    for (size_t i = 0; i < n; ++i)
    {
      ObjectPtr result = BaseClass::_this().getResult(i);
      assert(result);
      res += "\t" + lbcpp::toString(i) + ")\t" + result->getName() + "\t" + lbcpp::toString(BaseClass::_this().getResult(i)) + "\n";
    }
    return res;
  }
...
#endif // !LBCPP_CORE_IMPL_POLICY_STATIC_H_
