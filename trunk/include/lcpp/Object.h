...
#ifndef LCPP_OBJECT_H_
# define LCPP_OBJECT_H_
...
protected:
  ...
  static ReferenceCountedObjectPtr<T> checkCast(const std::string& where, ObjectPtr object)
  {
    ...
    if (object)
    {
      ...
      if (!res)
        error(where, "Could not cast object into '" + lcpp::toString(typeid(*res)) + "'");
    }
    return res;
  }
...
#define DECLARE_LCPP_CLASS(Name) \
  lcpp::Object::declare(lcpp::toString(typeid(Name)), lcpp::ObjectConstructor_<Name>::construct)
