...
public:
  virtual void progressCallback(const ProgressionStatePtr& progression)
  {
    if (progression->getUnit() == T("Learning Iterations")) {
      std::cout << "HERE:" << progression->getNormalizedValue() << std::endl;
    }
    //double globalProgression = computeGlobalProgression(progression);
    //writeProgressionFile(globalProgression);
  }
...
private:
  double computeGlobalProgression(const ProgressionStatePtr& progression)
  {
    return 0;
    
    /*
    // version de base:
    return progression->isBounded() ? progression->getNormalizedValue() : 0.0;
    
    // version hard-codé pour ProteinLearner:
    if (progression->getUnit() == T("Learning Iterations"))
      return progression->getNormalizedValue();
    else
      ...
      }*/
  }
